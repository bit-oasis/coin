 ...
 */
class KavaAddress implements CryptocurrencyAddress {
	...
	public function __construct(string $address, Cryptocurrency $currency, CryptocurrencyNetwork $cryptocurrencyNetwork) {
		if (!$this->isValid($address)) {
			throw new InvalidAddressException('This is not valid kava address - ' . $address);
		}
	...
	private function isValidBech32Address(string $address): bool {
		return (new KavaBech32AddressValidator($address))->validate();
	}
