...
use BitOasis\Coin\Address\Validators\NearAddressValidator;
 ...
 */
class NearAddress implements CryptocurrencyAddress {
	...
	public function __construct($address, Cryptocurrency $currency, CryptocurrencyNetwork $cryptocurrencyNetwork) {
		if (!$this->isValid($address)) {
			throw new InvalidAddressException('This is not valid Near address - ' . $address);
		}
	...
	private function isValid($address) {
		return (new NearAddressValidator($address))->validate();
	}
