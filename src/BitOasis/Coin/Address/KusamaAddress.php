...
use BitOasis\Coin\Address\Validators\KusamaAddressValidator;
 ...
 */
class KusamaAddress implements CryptocurrencyAddress {
	...
	public function __construct($address, Cryptocurrency $currency, CryptocurrencyNetwork $cryptocurrencyNetwork) {
		if (!$this->isValid($address)) {
			throw new InvalidAddressException('This is not valid Kusama address - ' . $address);
		}
	...
	private function isValid($address) {
		return (new KusamaAddressValidator($address))->validate();
	}
