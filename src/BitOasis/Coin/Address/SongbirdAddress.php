...
use BitOasis\Coin\Address\Validators\SongbirdAddressValidator;
 ...
 */
class SongbirdAddress implements CryptocurrencyAddress {
	...
	public function __construct(string $address, Cryptocurrency $currency, CryptocurrencyNetwork $cryptocurrencyNetwork) {
		if (!$this->isValid($address)) {
			throw new InvalidAddressException('This is not valid songbird address - ' . $address);
		}
	...
	public function isValid(string $address): bool {
		return (new SongbirdAddressValidator($address))->validate();
	}
}