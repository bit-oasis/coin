...
use BitOasis\Coin\Address\Validators\SolanaAddressValidator;
 ...
 */
class SolanaAddress implements CryptocurrencyAddress {
	...
	public function __construct($address, Cryptocurrency $currency, CryptocurrencyNetwork $cryptocurrencyNetwork) {
		if (!$this->isValid($address)) {
			throw new InvalidAddressException('This is not valid Solana address - ' . $address);
		}
	...
	private function isValid($address) {
		return (new SolanaAddressValidator($address))->validate();
	}
