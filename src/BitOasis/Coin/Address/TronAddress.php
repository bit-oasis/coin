...
use BitOasis\Coin\Address\Validators\TronAddressValidator;
 ...
 */
class TronAddress implements CryptocurrencyAddress {
	...
	public function __construct($address, Cryptocurrency $currency, CryptocurrencyNetwork $cryptocurrencyNetwork) {
		if(!$this->isValid($address)) {
			throw new InvalidAddressException('This is not valid Tron address - ' . $address);
		}
	...
	private function isValid($address) {
		return (new TronAddressValidator($address))->validate();
	}
