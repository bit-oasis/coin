...
use BitOasis\Coin\Address\Validators\SuiAddressValidator;
 ...
 */
class SuiAddress implements CryptocurrencyAddress {
	...
	public function __construct($address, Cryptocurrency $currency, CryptocurrencyNetwork $cryptocurrencyNetwork) {
		if (!$this->isValid($address)) {
			throw new InvalidAddressException('This is not valid sui address - ' . $address);
		}
	...
	public function isValid(string $address): bool {
		return (new SuiAddressValidator($address))->validate();
	}
}