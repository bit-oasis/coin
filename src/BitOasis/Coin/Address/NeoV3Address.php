...
use BitOasis\Coin\Address\Validators\NeoV3AddressValidator;
 ...
 */
class NeoV3Address implements CryptocurrencyAddress {
	...
	public function __construct($address, Cryptocurrency $currency) {
		if (!$this->isValid($address)) {
			throw new InvalidAddressException('This is not valid NEO address - ' . $address);
		}
	...
	private function isValid($address) {
		return (new NeoV3AddressValidator($address))->validate();
	}
