...
class Crc16Parameters {
	...
	public function __construct(int $poly, int $init, bool $reflectIn, bool $reflectOut, int $xorOut, string $name) {
		$this->poly = $poly;
		$this->init = $init;
	...
	public static function fromArray(array $algorithmParams): Crc16Parameters {
		list($poly, $init, $reflectIn, $reflectOut, $xorOut, $name) = $algorithmParams;
		return new Crc16Parameters($poly, $init, $reflectIn, $reflectOut, $xorOut, $name);
	}
}